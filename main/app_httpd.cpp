...
// NOTICE: This file has been altered by the owner of the repository
...
#define STREAM_ONLY 0

// LED FLASH setup
#if CONFIG_LED_ILLUMINATOR_ENABLED

#define LED_LEDC_CHANNEL LEDC_CHANNEL_2 //Using different ledc channel/timer than camera
#define CONFIG_LED_MAX_INTENSITY 128

#define CONFIG_LED_LEDC_SPEED_MODE LEDC_LOW_SPEED_MODE
#define CONFIG_LED_LEDC_CHANNEL LED_LEDC_CHANNEL
...
static esp_err_t bmp_handler(httpd_req_t *req)
{
    ...
    if (!fb)
    {
        ESP_LOGE( TAG_ERROR,"Camera capture failed");
    ...
    if(!converted){
        ESP_LOGE( TAG_ERROR,"BMP Conversion failed");
        httpd_resp_send_500(req);
        return ESP_FAIL;
    }
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    } else
    {
        ...
        if (!out_buf) {
            ESP_LOGE( TAG_ERROR,"out_buf malloc failed");
            httpd_resp_send_500(req);
            return ESP_FAIL;
        }
        ...
        if (!s) {
            free(out_buf);
            ESP_LOGE( TAG_ERROR,"To rgb888 failed");
            httpd_resp_send_500(req);
            return ESP_FAIL;
        }
    ...
    if (!s) {
        ESP_LOGE( TAG_ERROR,"JPEG compression failed");
        httpd_resp_send_500(req);
        return ESP_FAIL;
    }
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
...
static esp_err_t index_handler(httpd_req_t *req)
{
    ...
    if (s != NULL) {
        ...
        if(s->id.PID == OV2640_PID) {
            ...
            ESP_LOGE( TAG_ERROR,"Camera sensor not found, expected ov2640");
            return httpd_resp_send_500(req);
        }
    } else {
        ESP_LOGE( TAG_ERROR,"Camera sensor not found");
        return httpd_resp_send_500(req);
    }
}
...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            else
            {
                if (fb->format == PIXFORMAT_RGB565
                    ...
                    fb = NULL;
                    if (!s) {
                        ESP_LOGE( TAG_ERROR,"fmt2jpg failed");
                        res = ESP_FAIL;
                    }
#if CONFIG_ESP_FACE_DETECT_ENABLED && ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
        ...
        if (res != ESP_OK)
        {
            ESP_LOGE( TAG_ERROR,"Send frame failed");
            break;
        }
        int64_t fr_end = esp_timer_get_time();
